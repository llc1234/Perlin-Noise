...
def generate_noise_map(width, height, scale, octaves, persistence, lacunarity):
    ...
    for y in range(height):
        for x in range(width):
            ...
            noise_map[x][y] = int(noise_height / octaves * 255)
    return noise_map

# --- Pygame Drawing ---

def draw_map(screen, noise_map):
    height, width = noise_map.shape
    for y in range(height):
        for x in range(width):
            value = int(noise_map[y][x])
            color = (255, 255, 255) if value > 45 else (0, 0, 0)
...
def main():
    pygame.init()
    screen = pygame.display.set_mode((WIDTH, HEIGHT))
    pygame.display.set_caption(f"Custom Perlin Noise Map (Seed: {SEED})")

    noise_map = generate_noise_map(WIDTH, HEIGHT, SCALE, OCTAVES, PERSISTENCE, LACUNARITY)
    draw_map(screen, noise_map)
    pygame.display.flip()
